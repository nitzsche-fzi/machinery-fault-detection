...
'''
Depthwise Seperable Convolutional Neural Network (DS-CNN). Good performer and
very light in terms of model size.
'''
...
def create_model(train_data, train_target, val_data, val_target, params):
    ...
    if params['has_batch_norm']:
        model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.SeparableConv2D(filters=16*params['filter_factor'], kernel_size=(3, 1), padding='same', activation='relu'))
